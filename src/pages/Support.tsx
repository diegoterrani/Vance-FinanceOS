...
const STATUS_STYLE: Record<string, string> = {
  ...
  resolved: 'bg-emerald-500/10 text-emerald-400', closed: 'bg-neutral-500/10 text-[var(--text-secondary)]',
};
...
export default function Support() {
  ...
  if (!ready) return <div className="min-h-screen grid place-items-center bg-[var(--bg-app)] text-white"><p className="text-xs text-[var(--text-secondary)] animate-pulse">Carregando…</p></div>;
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-app)] text-[var(--text-primary)] font-sans">
      <header className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-soft)]">
        ...
        <a href="/" className="flex items-center gap-1.5 text-xs text-[var(--text-secondary)] hover:text-white"><ArrowLeft size={14} /> Voltar ao app</a>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="space-y-4">
          <div className="p-4 rounded-xl bg-[var(--bg-card)] border border-[var(--border-soft)]">
            ...
            <button onClick={create} disabled={creating} className="mt-2 w-full flex items-center justify-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-lg bg-[var(--text-primary)] text-[var(--bg-app)] hover:bg-white disabled:opacity-50">
          ...
          <div className="rounded-xl bg-[var(--bg-card)] border border-[var(--border-soft)] overflow-hidden">
            <div className="px-4 py-2.5 border-b border-[var(--border-soft)] text-xs font-semibold flex items-center gap-1.5"><MessageSquare size={13} /> Meus chamados</div>
            <div className="divide-y divide-white/5 max-h-[55vh] overflow-y-auto">
              {tickets.map(t => (
                <button key={t.id} onClick={() => open(t)} className={`w-full text-left px-4 py-3 hover:bg-[var(--bg-card)] ${sel?.id === t.id ? 'bg-[var(--bg-card)]' : ''}`}>
              ...
              {tickets.length === 0 && <p className="px-4 py-6 text-center text-xs text-[var(--text-muted)]">Nenhum chamado ainda.</p>}
            </div>
          </div>
        </div>

        <div className="lg:col-span-2 rounded-xl bg-[var(--bg-card)] border border-[var(--border-soft)] min-h-[60vh] flex flex-col">
          {!sel ? <p className="m-auto text-xs text-[var(--text-muted)]">Selecione ou abra um chamado.</p> : (
            <>
              <div className="px-4 py-3 border-b border-[var(--border-soft)]"><p className="text-sm font-semibold">{sel.subject}</p><span className={`px-1.5 py-0.5 rounded text-[9px] ${STATUS_STYLE[sel.status] || ''}`}>{sel.status}</span></div>
              <div className="p-4 space-y-3 flex-1 overflow-y-auto">
                {msgs.map(m => (
                  <div key={m.id} className="text-xs p-2.5 rounded-lg bg-[var(--bg-input)] border border-[var(--border-soft)]">
                ...
                {msgs.length === 0 && <p className="text-xs text-[var(--text-muted)]">Descreva seu problema enviando a primeira mensagem.</p>}
              </div>
              {sel.status !== 'closed' && (
                <div className="p-3 border-t border-[var(--border-soft)] flex gap-2">
                  <input value={reply} onChange={e => setReply(e.target.value)} placeholder="Mensagem…" onKeyDown={e => e.key === 'Enter' && send()}
                    className="flex-1 bg-[var(--bg-input)] border border-[var(--border-soft)] rounded-lg px-3 py-2 text-xs outline-none" />
                  <button onClick={send} className="px-3 py-2 rounded-lg bg-[var(--text-primary)] text-[var(--bg-app)] hover:bg-white"><Send size={14} /></button>
                </div>
              )}
            </>
          )}
